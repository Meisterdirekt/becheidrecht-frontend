/**
 * AG09 — Frontend-Agent (Haiku · wöchentlicher Batch)
 * Erstellt GitHub Issues für Frontend-Verbesserungen.
 */
...
async function execute(ctx: AgentContext): Promise<AgentResult<BatchResult>> {
  ...
  const messages: Anthropic.MessageParam[] = [
    {
      role: "user",
      content: `Analysiere den folgenden Kontext und erstelle GitHub Issues für Frontend-Verbesserungen falls nötig:\n\n${ctx.documentText}`,
    },
  ];
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (const block of response.content) {
      ...
      if (block.name === "github_action") {
        ...
        const result = await executeGithubAction(input.title, input.body, input.labels ?? ["frontend"]);
...
export const ag09FrontendAgent: Agent<BatchResult> = {
  id: "AG09",
  name: "Frontend-Agent",
  model: () => HAIKU_MODEL,
  execute,
