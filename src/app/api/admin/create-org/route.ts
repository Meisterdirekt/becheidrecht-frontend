...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (orgError || !org) {
      reportError(new Error(orgError?.message ?? "Org creation failed"), { context: "admin-create-org" });
    ...
    if (memberError) {
      ...
      reportError(new Error(memberError.message), { context: "admin-create-org" });
    ...
    reportError(error instanceof Error ? error : new Error(String(error)), { context: "admin-create-org" });
    return NextResponse.json({ error: "Einrichtung konnte nicht erstellt werden." }, { status: 500 });
  }
}
