...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    reportError(error instanceof Error ? error : new Error(String(error)), { context: "einrichtung-status" });
    return NextResponse.json({ error: "Einrichtungsstatus konnte nicht geladen werden." }, { status: 500 });
  }
}
