...
export default function Home() {
  ...
  // Absolute Definition aller Texte pro Sprache
  ...
  return (
    <div className="min-h-screen bg-[#05070a] text-white font-sans">
      ...
      <main className="max-w-7xl mx-auto pt-16 px-6 pb-24 text-center">
        {/* ÜBERSCHRIFT & BESCHREIBUNG (DYNAMISCH) */}
        ...
        <div className="max-w-2xl mx-auto bg-[#0a0c10] border border-white/10 p-12 rounded-[2.5rem] shadow-2xl mb-20">
          {!isAnalyzed ? (
            <button onClick={() => setIsAnalyzed(true)} className="w-full bg-blue-600 hover:bg-blue-500 py-4 rounded-xl text-xl font-black transition-all">
              {t.upload}
            </button>
          ) : (
            <div className="text-left space-y-6">
              <div className="bg-blue-600/10 p-6 rounded-2xl border border-blue-500/20 flex gap-4 text-sm italic text-gray-300">
                ...
                <p>Analyse abgeschlossen. Ihr Schreiben steht bereit.</p>
              </div>
        ...
        {/* 4 ABO SÄULEN - KOMPAKTER (KLEINER) */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-16 max-w-6xl mx-auto text-left">
          {[
            ...
            <div key={i} className={`p-6 rounded-2xl border transition-all ${p.high ? 'bg-blue-600 border-blue-400 scale-105 z-10' : p.dark ? 'bg-[#002147] border-white/10' : 'bg-[#0d1117] border-white/5'}`}>
              ...
              <ul className="space-y-3 mb-8">
                {p.features.map((f, j) => (
                  <li key={j} className="flex items-center gap-2 text-[10px] font-bold uppercase opacity-80">
                    <CheckCircle2 size={12} className="text-white" /> {f}
                  </li>
                ))}
              </ul>
              <button className={`w-full py-2.5 rounded-lg font-black text-[10px] uppercase ${p.high || p.dark ? 'bg-white text-blue-900' : 'bg-blue-600 text-white'}`}>{p.btn}</button>
            </div>
          ))}
        </div>

        {/* ERKLÄR-KÄSTCHEN (DYNAMISCH) */}
